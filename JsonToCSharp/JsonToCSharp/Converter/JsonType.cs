...
namespace Xamasoft.JsonClassGenerator
{
    public class JsonType
    {
        ...
        private JsonTypeEnum GetCommonTypeEnum(JsonTypeEnum type1, JsonTypeEnum type2)
        {
            ...
            switch (type1)
            {
                case JsonTypeEnum.Guid:
                    if (IsNull(type2)) return JsonTypeEnum.Guid;
                    if (type2 == JsonTypeEnum.Guid) return type1;
                    break;
                case JsonTypeEnum.Boolean:
        ...
        private static JsonTypeEnum GetFirstTypeEnum(JToken token)
        {
            ...
            if (token != null)
            {
                JValue _value = token as JValue;
                if (_value != null && _value.Value != null)
                {
                    if (Guid.TryParse(((JValue)token).Value.ToString(), out _guid))
                    {
            ...
            switch (type)
            {
                ...
                case JTokenType.Guid: return JsonTypeEnum.Guid;
